...
| DeepSeek-R1 | $0.55 | $2.19 | 深度推理（思考链） |
...
```
                     ┌─ 简单任务 → GPT-4o Mini (0.15/1M)
用户请求 → 路由判断 ─┼─ 中等任务 → GPT-4o (2.50/1M)
                     └─ 复杂任务 → Claude 4.5 Sonnet (15.00/1M)
```
...
**要点总结**
- Token 成本由输入和输出两部分构成，输出比输入贵 3-4 倍，System Prompt 每次调用都算进去
- 不同模型价格差异巨大，GPT-4o Mini 比 GPT-4o 便宜约 15 倍
- 七个省钱策略：选对模型等级、精简 System Prompt、控制输出长度、实现缓存、批量处理、Token 预算、设置告警
- 分层模型架构按任务复杂度选模型，混合模型在不同阶段用不同模型
- 成本监控记录每次调用数据，找到真正的优化方向
