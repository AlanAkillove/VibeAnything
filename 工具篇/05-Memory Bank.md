...
- AI: SiliconFlow API（DeepSeek V4 Flash + BAAI/bge-large-zh-v1.5 做 Embedding，国内直接可用）
- 部署: Vercel
...
## AI API（SiliconFlow，国内直接可用）
- embedding: BAAI/bge-large-zh-v1.5（中文优化）
- chat: deepseek-ai/DeepSeek-V4-Flash（摘要功能）
...
1. 为你当前正在做的一个项目创建 `memory-bank/` 目录，先写 `project.md`（技术栈、目录结构、核心约定）和 `decisions.md`（至少记录 2 个你做过的重要技术决策及理由）
2. 开一个全新的 AI 对话，第一句话就是"先读 memory-bank/ 了解项目全貌"，然后让 AI 基于读取的内容继续开发——验证 Memory Bank 是否真的够用
3. 在 `decisions.md` 里记录一个"被否决的方案及原因"——下次 AI 再提出类似建议时，看它会不会读到这份记录并避免重复
4. 完成一个小功能后，更新 `progress.md`——标记已完成项、当前进行项、待办项。下次开新对话时验证 AI 是否能准确读取进度
5. 尝试让 AI 在 commit 前自动更新 Memory Bank：把代码变更贴给 AI，让它检查是否有架构变化、新依赖、或进度更新
...
- **上一篇文章**：[04-常见陷阱与调试技巧](./04-常见陷阱与调试技巧.md) — Memory Bank 如何帮助你避免那些陷阱
- **相关阅读**：[04-上下文窗口管理](../AI协作心法篇/04-上下文窗口管理.md) — Memory Bank 的理念来源和更深层的上下文管理
- **相关阅读**：[06-AI会话管理](../AI协作心法篇/06-AI会话管理.md) — 如何配合 Memory Bank 管理多个 AI 会话
- **相关阅读**：[04-Obsidian与Markdown工作流](../环境准备篇/04-Obsidian与Markdown工作流.md) — 本地 Markdown 工具推荐
