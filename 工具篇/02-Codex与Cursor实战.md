...
1. 安装 Cursor：去 cursor.com 下载，用 GitHub 账号登录，打开你最近的项目等 Indexing 完成（右下角进度条），然后创建 `.cursorrules` 文件写入你的技术栈约定
2. 在 Cursor 中完成一次完整流程：用 Cmd+L（Chat）理解一个函数 → 用 Cmd+I（Composer）给它加一个新功能 → 用 `git diff` 检查改动
3. 安装 Codex CLI：`npm install -g @openai/codex`，在项目目录下运行 `codex`，用自然语言描述一个小功能让它生成并运行
4. 在两个工具上各做一个相同的任务（比如创建一个 Markdown 解析脚本），对比体验：哪个更快？哪个更精准？哪个更适合你的工作习惯？
5. 建立一个铁律：每完成一个可用功能点就 git commit —— 不管用哪个工具，Commit 习惯是面对 AI 修改的唯一回滚按钮
...
- [Cursor 官方文档](https://docs.cursor.com) — 完整功能说明和快捷键参考
- [Codex CLI GitHub](https://github.com/openai/codex) — 安装指南和命令参考
- **上一篇文章**：[01-主流AI编码工具对比](./01-主流AI编码工具对比.md) — 为什么选这两个工具
- **下一篇文章**：[03-AI工具组合拳](./03-AI工具组合拳.md) — 如何组合多个工具发挥最大效能
- **相关阅读**：[04-常见陷阱与调试技巧](./04-常见陷阱与调试技巧.md) — 使用这些工具时最常见的坑
- **相关阅读**：[05-Memory Bank](./05-Memory Bank.md) — 跨工具共享上下文的解决方案
